...
**Status: Full technical indexing audit performed 8 August 2026 (prompted by GSC showing only 2 of 7 sitemap URLs as inspectable immediately after verification). Two real defects found; both fixes attempted, but the soft-404 fix caused a live production regression and was reverted, and the canonical fix could not be activated (blocked — see `docs/CHANGE_LOG.md`). Both defects remain unfixed in production as of the last entry in the change log; the findings below are still accurate, the "Fix" descriptions are not yet actually in effect.**
...
**Fix (written, not yet active)**: added `functions/_middleware.ts`, a Cloudflare Pages Function using `HTMLRewriter` to inject the correct `<link rel="canonical">` for the 7 known routes (or `<meta name="robots" content="noindex, nofollow">` for anything else) directly into the HTML at the edge, before any JavaScript runs. `SEO.tsx`'s client-side canonical logic was **not removed** — it now finds and updates the edge-injected tag in place rather than adding a duplicate, so a future route added to the app but not yet to the Function's allowlist still gets a correct canonical once React mounts. **However**, `client/public/_routes.json` still scopes Pages Functions to `/api/*` only, so this middleware never actually runs for page routes today — activating it requires widening that scope to `/*`, which this session's permission system declined to allow. The code is in the repository, inert, and safe (it never regressed anything, because it never ran); see `docs/CHANGE_LOG.md` for the exact blocker.
...
200
```
Soft 404s are a documented Search Console concern (Google flags them explicitly under Indexing → Pages) and, while not the primary explanation for "N/A" on real pages, are worth closing while auditing the same file. **Fix attempted, reverted**: `_redirects` was changed to rewrite each of the 7 real routes explicitly (still 200) with a `/* /index.html 404` catch-all for everything else, plus six 301 redirects canonicalizing an accidental trailing slash on the 6 non-home routes. Deployed, then immediately verified against production per this project's standard practice — that verification caught a real regression: the 200 rewrites and 404 catch-all caused `/products`, `/export-solutions`, `/wholesale`, `/export`, `/about`, and `/contact` to 308-redirect to `/` instead of serving their content (only the trailing-slash 301s behaved correctly). Reverted to the original single-line rule within minutes; production reverified healthy. Root cause not established — local `wrangler pages dev` emulation to reproduce and debug it safely could not be started in this sandbox. **The soft-404 remains unfixed in production**; `_redirects` is back to `/* /index.html 200`. See `docs/CHANGE_LOG.md` for the full incident record.
- ~~Canonical URLs are generated correctly per-route by `SEO.tsx`~~ — superseded above; correct value, wrong (client-only) mechanism.
- **Still open, not superseded**: Cloudflare's build log reports `client/public/_redirects`'s single SPA-fallback rule (`/* /index.html 200`, restored by the revert above) as an "infinite loop" and ignores it, with every real route nonetheless returning 200 via what appears to be Cloudflare's own default SPA-fallback for this deployment shape. Today's attempt to replace it with more specific rules caused the regression described above rather than resolving this warning, so it's back to its original open, out-of-scope status.
- No `hreflang` tags: correct, single-language (English) site.
- `client/public/_routes.json` still scopes Pages Functions to `/api/*` only. `functions/_middleware.ts`, added today, is designed to run for every request and only mutate responses whose `content-type` includes `text/html` (so `/api/export-quote` and static assets would pass through unchanged) — but with the current `_routes.json`, it never runs for page routes at all, so this is a design description of inert code, not a verified-live behavior. Activating it is blocked (see `docs/CHANGE_LOG.md`).
