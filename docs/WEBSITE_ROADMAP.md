...
Config-gated GA4 tracking and RFQ conversion events, config-gated Search Console/Bing verification tags, a real structured-data bug fix (product schema was built but never rendered), sitemap/robots.txt validation, and Core Web Vitals fixes (removed ~7.9 MB of orphaned images, re-encoded product photos to JPEG for an ~88% size reduction, made the fonts stylesheet non-render-blocking) are all live. Two items remain genuinely blocked on external account access this repository does not have: completing Search Console/Bing verification (needs a BorgaFoods-controlled Google/Microsoft account) and activating GA4 (needs a GA4 property to be created). A recommended recurring "Growth Audit" was designed but could not be scheduled as a cloud routine — blocked by GitHub not being connected to the claude.ai connector used for that feature.
